...
# https://github.com/sofiatolaosebikan/hopcroftkarp/blob/4ecacde92237af7a4750a5a6818cd26aeebb28df/hopcroftkarp/__init__.py
...
hh = set()
hrev = defaultdict(lambda: [])
for n in range(N):
    for k in range(K):
        x = h[n][k]
        hh.add(x)
        for t in range(a[k]):
            hrev[x].append((n+K*N, k+t*K))
hh = sorted(list(hh))

def is_possible_graph(graph):
    return len(HopcroftKarp(graph).maximum_matching(keys_only=True)) == N
...
graph = {}
while True:
    if high >= 0 and is_possible_graph(graph):
        bestdiff = min(bestdiff, hh[high]-hh[low])
        for (i,j) in hrev[hh[low]]:
            graph[i].remove(j)
            if len(graph[i]) == 0:
                del graph[i]
        low += 1
